...
# Этот хэндлер будет срабатывать на команду "/ending"
# и отправлять пользователю первую страницу книги с кнопками пагинации
@router.message(Command(commands='ending'))
async def process_ending_command(message: Message):
...
async def process_continue_command(message: Message):
    if users_db[message.from_user.id]['page'] == 1:
        text = book[users_db[message.from_user.id]['page']]
        await message.answer(
                text=text,
                reply_markup=create_pagination_keyboard(
                    f'{users_db[message.from_user.id]["page"]}/{len(book)}',
                    'forward'))
    elif users_db[message.from_user.id]['page'] == len(book):
        text = book[users_db[message.from_user.id]['page']]
        await message.answer(
                text=text,
                reply_markup=create_pagination_keyboard(
...
async def process_forward_press(callback: CallbackQuery):
    if users_db[callback.from_user.id]['page'] == len(book) - 1:
...
async def process_backward_press(callback: CallbackQuery):
    if users_db[callback.from_user.id]['page'] <= 2:
        users_db[callback.from_user.id]['page'] -= 1
        text = book[users_db[callback.from_user.id]['page']]
        await callback.message.edit_text(
                text=text,
                reply_markup=create_pagination_keyboard(
                    f'{users_db[callback.from_user.id]["page"]}/{len(book)}',
                    'forward'))
    elif users_db[callback.from_user.id]['page'] > 2:
        users_db[callback.from_user.id]['page'] -= 1
        text = book[users_db[callback.from_user.id]['page']]
        await callback.message.edit_text(
                text=text,
